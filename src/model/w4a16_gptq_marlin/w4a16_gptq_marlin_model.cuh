...
struct W4A16GPTQMarlinModelImpl: Model {
    ...
    W4A16GPTQMarlinModelImpl(
        ...
        for (int i = 0; i < num_hidden_layers; i++) {
            layers.push_back(new W4A16GPTQMarlinLayer<T>(hidden_size, intermediate_size, num_attention_heads, num_key_value_heads, head_dim, rms_norm_eps, group_size, residual_scale, 0, use_qk_norm, use_attn_bias));
        }
