    ...
    def generate(self, input_ids, generation_length=100, teminators=[], use_stream=False, progress_callback=None):
        ...
        if use_stream:
            ...
            while i < generation_length-1 and not terminal:
                ...
                # print(f"tree_draft_ids: {self.tree_draft_ids}")
                # print(f"tree_draft_ids.shape: {self.tree_draft_ids.shape}")
                # print(f"tree_position_ids: {self.tree_position_ids}")
                # print(f"tree_position_ids.shape: {self.tree_position_ids.shape}")
                # print(f"cache_length: {self.cache_length}")
                # print(f"cache_length.shape: {self.cache_length.shape}")
                # print(f"tree_attn_mask: {self.tree_attn_mask}")
                # print(f"tree_attn_mask.shape: {self.tree_attn_mask.shape}")
                # print(f"tree_parent: {self.tree_parent}")
                # print(f"tree_parent.shape: {self.tree_parent.shape}")
                logits = self.decode(self.tree_draft_ids, self.tree_position_ids, self.cache_length, mask_2d=self.tree_attn_mask)
                if self.temperature > 0.0:
                    self.tree_gt_ids.copy_(torch.multinomial(F.softmax(logits/self.temperature, dim=-1), num_samples=1, generator=self.generator).squeeze(-1))
                else:
