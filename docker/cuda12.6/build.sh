...
DEFAULT_BASE_IMAGE="nvcr.io/nvidia/cuda:12.6.0-devel-ubuntu22.04"
...
if [ -n "$BASE_IMAGE" ]; then
    ...
    if docker image inspect nvcr.io/nvidia/cuda:12.6.0-devel-ubuntu22.04 >/dev/null 2>&1; then
...
echo "docker build $DOCKER_FLAGS $BUILD_ARGS -t $IMAGE_TAG ."
docker build $DOCKER_FLAGS $BUILD_ARGS -t $IMAGE_TAG .
...
echo "./build.sh \"\" nvidia/cuda:12.6.0-devel-ubuntu22.04  # use Docker Hub image"
echo "./build.sh \"86,89\" nvcr.io/nvidia/cuda:12.6.0-devel-ubuntu22.04  # full custom"
echo ""
echo "=== Base image options ==="
echo "nvcr.io/nvidia/cuda:12.6.0-devel-ubuntu22.04  # NVIDIA optimized (recommended)"
echo "nvidia/cuda:12.6.0-devel-ubuntu22.04          # Docker Hub standard"
echo ""
